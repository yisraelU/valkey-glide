...
/**
 * Transaction implementation for JSON module. Transactions allow the execution
 * of a group of commands in a single step. See {@link Transaction}.
 *
 * @example
 *     <pre>{@code
...
public class MultiJson {
     ...
     * @return Command Response - A simple <code>"OK"</code> response if the value is successfully set.
     */
    public static <ArgType, T extends BaseTransaction<T>> BaseTransaction<T> set(
        ...
        transaction.customCommand(newArgsBuilder().add(JSON_SET).add(key).add(path).add(value).toArray());
        return transaction;
    }
     ...
     * @return Command Response - A simple <code>"OK"</code> response if the value is successfully set. If value isn't
    ...
    public static <ArgType, T extends BaseTransaction<T>> BaseTransaction<T> set(
        ...
        checkTypeOrThrow(path);
        checkTypeOrThrow(value);
        transaction.customCommand(newArgsBuilder().add(JSON_SET).add(key).add(path).add(value).add(setCondition.getValkeyApi()).toArray());
        return transaction;
    }
