...
impl<Connection> From<(Connection, Option<IpAddr>, Option<String>)> for ConnectionWithIp<Connection> {
    ...
    Connection: Clone,
{
    ...
    pub(crate) fn az_for_address(
        &self,
        address: &str,
    ) -> Option<String> {
        self.connection_map.get(address).map(|item| {
            item.value().user_connection.az.clone()
        })?
    }
